...
<div class="mb-8 flex items-center gap-3">
    ...
    <div>
        <h1 class="text-2xl font-black font-['Space_Grotesk'] text-slate-900 mb-1">Tambah Pengalaman</h1>
        <p class="text-slate-500 text-xs">Tambahkan riwayat perjalanan karir atau pencapaian baru ke portofolio Anda.</p>
    </div>
</div>
...
<div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm max-w-4xl">
    <form action="{{ route('admin.experiences.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div class="form-group mb-0">
                ...
                <select name="color" class="form-control cursor-pointer">
                    <option value="indigo" {{ old('color') == 'indigo' ? 'selected' : '' }}>Indigo (Biru Keunguan)</option>
                    <option value="cyan" {{ old('color') == 'cyan' ? 'selected' : '' }}>Cyan (Biru Muda)</option>
                    <option value="purple" {{ old('color') == 'purple' ? 'selected' : '' }}>Purple (Ungu)</option>
                    <option value="emerald" {{ old('color') == 'emerald' ? 'selected' : '' }}>Emerald (Hijau)</option>
                    <option value="rose" {{ old('color') == 'rose' ? 'selected' : '' }}>Rose (Merah Muda)</option>
                    <option value="amber" {{ old('color') == 'amber' ? 'selected' : '' }}>Amber (Kuning Keemasan)</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group mb-6">
            ...
            <textarea name="description" class="form-control" rows="4" required placeholder="Ceritakan apa saja tanggung jawab, tantangan, atau pencapaian Anda selama periode ini...">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="form-group mb-0">
                ...
                <input type="text" name="tags" class="form-control" value="{{ old('tags') }}" placeholder="Laravel, Vue.js, MySQL, API">
