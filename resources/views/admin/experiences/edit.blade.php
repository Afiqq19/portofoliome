    ...
    <form action="{{ route('admin.experiences.update', $experience->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="form-group mb-0">
                ...
                <input type="text" name="title" class="form-control" value="{{ old('title', $experience->title) }}" required placeholder="Contoh: Freelance Fullstack Developer / D3 Manajemen Informatika">
            </div>
            
            <div class="form-group mb-0">
                <label class="form-label">Nama Institusi / Perusahaan / Organisasi</label>
                <input type="text" name="company" class="form-control" value="{{ old('company', $experience->company) }}" placeholder="Contoh: PT Pelindo Multi Terminal / Politeknik Negeri Medan">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="form-group mb-0">
                ...
                <select name="category" class="form-control cursor-pointer" required>
                    <option value="work" {{ old('category', $experience->category ?? 'work') == 'work' ? 'selected' : '' }}>💼 Karir & Magang (Work & Internship)</option>
                    <option value="education" {{ old('category', $experience->category ?? 'work') == 'education' ? 'selected' : '' }}>🎓 Pendidikan (Education)</option>
                    <option value="organization" {{ old('category', $experience->category ?? 'work') == 'organization' ? 'selected' : '' }}>🏛️ Organisasi & Kepemimpinan</option>
                </select>
            </div>

            <div class="form-group mb-0">
                <label class="form-label">Periode Waktu <span class="text-rose-500">*</span></label>
                <input type="text" name="period" class="form-control" value="{{ old('period', $experience->period) }}" required placeholder="Contoh: 2024 - Sekarang atau Jan 2026 - Mar 2026">
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="form-group mb-0">
                ...
                <input type="number" name="order" class="form-control" value="{{ old('order', $experience->order) }}" min="1" required>
                <p class="text-xs text-slate-500 mt-1">Nomor urutan tampil di web (Dimulai dari angka 1 ke atas).</p>
            </div>
        </div>
